...
class ReadEdges():
    ...
    def write_mat_to_file(self, mat):
        """
        Writes the laplacian to a file for the cuda script to grab and calculate
        """
        filename = ROOT_DIR + '/' + self.filename.split('/')[-1] + '.laplacian.txt'
        rows = mat.shape[0]
        cols = mat.shape[1]
        with open(filename, 'w', newline = '\n') as mat_fl:
            mat_fl.write(str(rows) + ' ' + str(cols) + '\n')
            for i in range(rows):
                for k in range(cols):
                    mat_fl.write(str(int(mat[i, k])) + ' ')
                mat_fl.write('\n')
